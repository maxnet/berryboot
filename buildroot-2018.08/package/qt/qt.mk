...
QT_CFLAGS = $(TARGET_CFLAGS) -flto -fuse-linker-plugin
QT_CXXFLAGS = $(TARGET_CXXFLAGS) -flto -fuse-linker-plugin
QT_LDFLAGS = $(TARGET_LDFLAGS) -flto -fuse-linker-plugin -Os
QT_CONFIGURE_OPTS += -no-neon

# Cut down features a bit
QT_CONFIGURE_OPTS += -no-decoration-windows \
	-no-feature-CONCURRENT -no-feature-DRAGANDDROP -no-feature-TABLETEVENT \
	-no-feature-WORKSPACE -no-feature-MDIAREA -no-feature-SIGNALMAPPER -no-feature-TABDIALOG \
	-no-feature-DATAWIDGETMAPPER -no-feature-IMAGEFORMATPLUGIN -no-feature-MOVIE -no-feature-IMAGEFORMAT_PPM \
	-no-feature-IMAGEFORMAT_XBM -no-feature-IMAGEFORMAT_BMP \
	-no-feature-IMAGE_HEURISTIC_MASK -no-feature-DIRECTPAINTER -no-feature-PICTURE \
	-no-feature-COLORNAMES -no-feature-RASTERCALLBACKS -no-feature-TEXTCODEC -no-feature-QWS_INPUTMETHODS \
	-no-feature-STATEMACHINE -no-feature-GESTURES -no-feature-QWS_MULTIPROCESS -no-feature-QWSEMBEDWIDGET \
	-no-feature-SOUND -no-feature-XMLSTREAM -no-feature-TABLEVIEW \
	-no-feature-CALENDARWIDGET -no-feature-TEXTBROWSER \
	-no-feature-SPLASHSCREEN -no-feature-LCDNUMBER \
	-no-feature-FONTCOMBOBOX -no-feature-DIAL -no-feature-GRAPHICSVIEW \
	-no-feature-SPINWIDGET -no-feature-SYNTAXHIGHLIGHTER -no-feature-SIGNALMAPPER \
	-no-feature-WHATSTHIS -no-feature-COLORDIALOG -no-feature-FONTDIALOG \
	-no-feature-PRINTDIALOG -no-feature-PRINTPREVIEWDIALOG -no-feature-WIZARD \
	-no-feature-PRINTER -no-feature-PAINT_DEBUG -no-feature-QWS_QPF \
	-no-feature-QWS_QPF2 \
	-no-feature-FTP -no-feature-HTTP -no-feature-NETWORKPROXY -no-feature-NETWORKDISKCACHE \
	-no-feature-BEARERMANAGEMENT -no-feature-URLINFO -no-feature-HOSTINFO
...
define QT_CONFIGURE_CMDS
	...
	$(call QT_QMAKE_SET,QMAKE_AR,$(TARGET_CROSS)gcc-ar cqs,$(@D))
	$(call QT_QMAKE_SET,QMAKE_OBJCOPY,$(TARGET_OBJCOPY),$(@D))
	$(call QT_QMAKE_SET,QMAKE_RANLIB,$(TARGET_CROSS)gcc-ranlib,$(@D))
